...
{
    public class ResultLinqAsyncAndSync
    {
        public record Error(string msg);
        public record ValidationError(string msg) : Error(msg);
        public record SavedUserError(string msg) : Error(msg);
        ...
        public static Result<User, Error> ValidateUser(string username)
        {
            ...
            {
                return Result.Failure<User, Error>(new ValidationError($"username includes illegal char"));

            }
            return Result.Success<User, Error>(new User(username));
        }
        ...
        public static async Task<Result<SavedUsed, Error>> SaveUser(User user)
        {
            ...
            {
                return await Task.FromResult(Result.Failure<SavedUsed, Error>(new SavedUserError($"SaveUserError")));
